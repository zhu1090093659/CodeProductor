...
- 機能強化：Mem0 サービスを ProcessConfig と v2 API に更新し、メモリ管理を改善。

### 1.7.15

- 機能強化：Mem0 メモリサービス統合により、エージェントの永続的メモリをサポート。
- 機能強化：CLI インストール管理を実装し、ツールセットアップを効率化。
- 機能強化：画像生成ツール統合を導入。
- 修正：ビルド・リリースワークフローのタグ処理ロジックを更新。

### 1.7.13

- 機能強化：カスタムアシスタントの作成と管理を追加。
- 機能強化：Guid ページに Agent の選択と分類を追加。
- 機能強化：スキル削除機能を追加。
- 修正：テーマ初期化を DEFAULT_THEME に変更し、システム判定依存を削減。
- ビルド/雑務：About のリンクを新しいリポジトリに更新。
- ビルド/雑務：electron-builder の設定を更新しインストーラ挙動を改善。
